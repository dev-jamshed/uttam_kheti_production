                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-4">
                                    ...
                                    <select class="form-control select2" name="city_ids[]" class="w-100" id="city_ids"
                                        data-toggle="select2" data-placeholder="{{ localize('Select cities') }}" disabled>
                                ...
                                <div class="mb-4">
                                    @php
                                        $logisticAreas = $logisticZone->areas->pluck('area_id')->toArray();
                                    @endphp

                                    <label class="form-label">{{ localize('Areas') }}</label>
                                    <select class="form-control select2" name="area_ids[]" class="w-100" id="area_ids"
                                        data-toggle="select2" data-placeholder="{{ localize('Select areas') }}" multiple required>
                                        @foreach ($cities as $city)
                                            @foreach ($city->areas as $area)
                                                <option value="{{ $area->id }}"
                                                    {{ in_array($area->id, $logisticAreas) ? 'selected' : '' }}>
                                                    {{ $area->name }}</option>
                                            @endforeach
                                        @endforeach
                                    </select>
